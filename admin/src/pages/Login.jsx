...
function Login() {
    ...
    const AdminLogin = async (e) => {
        ...
        try {
            ...
            console.error(error)
            toast.error(error.response?.data?.message || "Admin Login Failed: Invalid credentials or server error")
        }
        setLoading(false)
    }
        ...
        <div className='w-[100vw] h-[100vh] bg-gray-50 flex items-center justify-center'>
            <div className='w-[92%] max-w-[420px] bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden'>
                ...
                <div className='flex flex-col items-center pt-[36px] pb-[10px] gap-[8px]'>
                    <div className='text-4xl font-black text-black tracking-tight uppercase'>School Portal</div>
                    <span className='text-[11px] font-semibold text-gray-400 uppercase tracking-widest bg-gray-100 px-3 py-1 rounded-full'>School Admin</span>
                    <h1 className='text-gray-900 text-[20px] font-bold mt-[4px]'>Admin Login</h1>
                    <p className='text-gray-400 text-[13px]'>Sign in to manage your school website</p>
                </div>
